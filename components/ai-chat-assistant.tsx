...
export function AIChatAssistant({ onActionExecuted }: AIChatAssistantProps) {
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      if (onActionExecuted) {
        onActionExecuted("chat_message", { message: textToSend, response: data.response })
      }
    } catch (error) {
          ...
          <div className="space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-muted-foreground py-8">
                ...
                <p className="text-xs">Ask me about your music library, playlists, or use the quick actions above.</p>
              </div>
            )}
